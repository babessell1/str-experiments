...
class EHDNExperiment:
    ...
    def collapse_variants(self, df):
        """
        match adjacent variants from a subject and aggregate row based on chromosome, proximity of the left coordinates and repeat unit and collapse the subject variant into the aggregate variant by updating the mean and variance with the welford online algorithm.
        """
    ...
    def summarize_ehdn(self, summary_csv=None):
        """
        ...
        for chrom in self.chroms:
            ...
            if iteration==1:
                ...
                dff["mean_left"] = dff["start"]
                dff["counts"] = np.ones(dff.shape[0])        
                dff["variance_left"] = np.zeros(dff.shape[0])
                dff["m2"] = np.zeros(dff.shape[0])
                #dff["mean_df"] = dff["left"]
                columns_to_drop = [
                    "end", "num_anc_irrs", "norm_num_anc_irrs", "het_str_size"
                ]
                dff.drop(columns=columns_to_drop, inplace=True)
                dff.rename(columns={"contig": "#chrom", "motif": "repeatunit", "start": "left", "het_str_size": "allele2_est"}, inplace=True)
                #print(dff)
            ...
            for file in self.tsvs[start_idx:]:
                ...
                next_df["mean_left"] = next_df["start"]
                next_df["m2"] = np.zeros(next_df.shape[0])
                next_df.drop(columns=columns_to_drop, inplace=True)
                next_df.rename(columns={"contig": "#chrom", "motif": "repeatunit", "start": "left", "het_str_size": "allele2_est"}, inplace=True)
                dff_before = dff
    ...
    def par_process_variant(self, variant_row):
        ...
        for tsv_file in self.tsvs:
            ...
            variant_df = tsv_df[tsv_df['motif'] == variant]
            add_variants = variant_df[
                (variant_df['start'] >= mean_left - range_ ) &
                (variant_df['start'] <= mean_left + range_ )
            ]['het_str_size'].tolist()
